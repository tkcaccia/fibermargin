...
identifies independent specimens. `workers = NULL` uses a conservative automatic
CPU budget; set an integer for an explicit budget.
...
The combined result is equivalent to refining each specimen separately.

```{r specimen-isolation, eval=FALSE}
joint <- refine_spatial_labels(
  gradient$xy, gradient$labels,
  samples = gradient$samples, workers = 2L
)

separate <- unsplit(
  lapply(split(seq_len(nrow(gradient$xy)), gradient$samples), function(rows) {
    refine_spatial_labels(
      gradient$xy[rows, , drop = FALSE],
      gradient$labels[rows],
      workers = 1L
    )
  }),
  gradient$samples
...
Changing `workers` changes execution only, not the mathematical result.
...
identical(as.character(one_worker), as.character(four_workers))
```
